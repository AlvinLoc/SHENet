...
class MOTreID(MOTSequence):
        ...
        if self.random_triplets:
            ...
            if self.P:
                ...
                neg_indices = np.random.choice(
                    [i for i, _ in enumerate(self.data) if i != idx],
                    self.P - 1,
                    replace=False,
                )
                for i in neg_indices:
                    neg = self.data[i]
                    res.append(
                        neg[np.random.choice(neg.shape[0], self.K, replace=False)]
                    )
        else:
            ...
            if self.P:
                ...
                neg_indices_choice = np.linspace(
                    0, len(self.data) - 2, num=self.P - 1, dtype=int
                )
                ...
                for i in neg_indices:
                    neg = self.data[i]
                    res.append(
                        neg[np.linspace(0, neg.shape[0] - 1, num=self.K, dtype=int)]
                    )
    ...
    def build_samples(self):
        ...
        if self._seq_name:
            self.logger(
                f"[*] Loaded {len(res)} persons from sequence {self._seq_name}."
            )
