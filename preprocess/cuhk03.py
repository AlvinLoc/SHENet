...
class CUHK03(Dataset):
    ...
    def __init__(
        ...
        if transform == "random":
            ...
            raise NotImplementedError(
                "Tranformation not understood: {}".format(transform)
            )
    ...
    def __getitem__(self, idx):
        ...
        neg_indices = np.random.choice(
            [i for i in range(len(self.data)) if i != idx], self.P - 1, replace=False
        )
        ...
        for campid, camp_ref in enumerate(mat[self.seq_name][0]):
            ...
            for pid in range(num_pids):
                ...
                for imgid, img_ref in enumerate(camp[pid, :]):
                    ...
                    sample = {"im": self.build_crop(img), "id": id_num}
                    total.append(sample)
    ...
    def build_samples(self):
        ...
        if self.seq_name:
            print(
                "[*] Loaded {} persons from sequence {}.".format(
                    len(res), self.seq_name
                )
            )
