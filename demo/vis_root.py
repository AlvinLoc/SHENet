...
def vis_img(img, root_pred, root_gt, save_path):
    ...
    cv2.putText(
        img, "pred trajectory", (5, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 255), 2
    )
    cv2.putText(
        img, "gt trajectory", (10, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 255, 0), 2
    )
...
def run(joint, idx, input_n, output_n, save_path):
    ...
    vis_img(
        img.copy(), root_pred, root_gt[: input_n + output_n], f"{save_path}/{idx}.jpg"
    )
